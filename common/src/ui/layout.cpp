    ...
    const std::vector<std::unique_ptr<widget>>& children) const
{
    ...
    for (const auto& child : children) {
        if (child->style.width > 0) {
            fixedTotal += static_cast<float>(child->style.width);
        } else {
            ++flexCount;
        }
    }
    ...
    for (auto& child : children) {
        ...
        if (child->style.width > 0) {
            ...
            child->x = currentX;
            child->width = static_cast<float>(child->style.width);
    ...
    const std::vector<std::unique_ptr<widget>>& children) const
{
    ...
    for (const auto& child : children) {
        if (child->style.height > 0) {
            fixedTotal += static_cast<float>(child->style.height);
        } else {
            ++flexCount;
        }
    }
    ...
    for (auto& child : children) {
        ...
        if (child->style.height > 0) {
            ...
            child->y = currentY;
            child->height = static_cast<float>(child->style.height);
