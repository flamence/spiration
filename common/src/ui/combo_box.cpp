...
void combo_box::paint(std::shared_ptr<renderer> renderer) {
    renderer->draw_rectangle({x, y, width, height}, theme_manager::get(theme_manager::COMBO_BG));
    renderer->draw_rectangle_outline({x, y, width, height}, theme_manager::get(theme_manager::COMBO_BORDER), 1.5f);
    ...
    if (expanded_) {
        float py = y + height;
        renderer->draw_rectangle({x, py, width, popup_height()}, theme_manager::get(theme_manager::POPUP_BG));
        renderer->draw_rectangle_outline({x, py, width, popup_height()}, theme_manager::get(theme_manager::POPUP_BORDER), 1.0f);

        for (size_t i = 0; i < items.size(); ++i) {
            float iy = py + i * item_height;
            if (static_cast<int>(i) == hovered_idx_) {
                renderer->draw_rectangle({x + 1.0f, iy, width - 2.0f, item_height},
                                         theme_manager::get(theme_manager::POPUP_HOVER));
            }
