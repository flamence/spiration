...
void combo_box::paint(std::shared_ptr<renderer> renderer) {
    ...
    renderer->draw_rectangle_outline({0, 0, width, height}, theme_manager::get(theme_manager::COMBO_BORDER), 1.5f);

    std::string display = (selected_index >= 0 && selected_index < static_cast<int>(items.size()))
                              ? items[selected_index] : "";
    renderer->draw_text_aligned(display, {8.0f, 0, width - 24.0f, height},
    ...
    if (expanded_) {
        float py = height;
        renderer->draw_rectangle({0, py, width, popup_height()}, theme_manager::get(theme_manager::POPUP_BG));
        renderer->draw_rectangle_outline({0, py, width, popup_height()}, theme_manager::get(theme_manager::POPUP_BORDER), 1.0f);
